...
- **Night falls in Dallas.** Sunrise and sunset are computed locally for Dallas (NOAA solar equations in `game/solar.ts`, no network) and between them the room and water darken and every landmark switches to its night look: the castle's windows glow with a candle flicker, Reunion Tower's ball runs lamp programs (steady → chase → sweep), the Eiffel Tower turns golden and sparkles for half a minute every fifth minute, Big Ben's dial glows, the Parthenon and Stonehenge are floodlit, City Hall's offices light up, the pineapple gets a chasing string of coloured bulbs, the skyline's argon and spire come on, and the Omni's sign goes neon. The clock stays lit.
