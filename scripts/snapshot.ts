...
import { describeWeatherCode } from "../src/game/omni";
...
// Omni extras for headless renders: OMNI_MESSAGE="GO STARS" OMNI_WEATHER="82,2" (°F, WMO code)
const wx = process.env.OMNI_WEATHER?.split(",");
const weather = wx && wx.length === 2 ? { tempF: Number(wx[0]), ...describeWeatherCode(Number(wx[1])), at: 0 } : null;
engine.setInputs({ mood: mood as FishMood, cleanliness: Number(clean), happiness: 80, timeFormat: fmt as TimeFormat, structure, species, tank, omniMessage: process.env.OMNI_MESSAGE ?? "", weather });
// Drive the private loop manually with fixed steps.
type Priv = { tick(dt: number): void; render(): void; feed(): void };
const p = engine as unknown as Priv;
p.feed();
const steps = Math.round(Number(secs) / (1 / 60));
for (let i = 0; i < steps; i++) p.tick(1 / 60);
p.render();
